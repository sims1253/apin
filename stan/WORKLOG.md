...
## 2026-08-29 — W-128 CLOSE-OUT (PI-arbitrated): Lane B — glm emission for everyday forms — ALL FIVE GATES GREEN, band exceeded favorably: kidscore likelihood subtree 54.3→9.5 Ir/elem = −82.5% (band −40..−60; overshoot owned: the census's 18-20 target was the K=2 var-beta shape — the K=1 all-scalar Map form is cheaper), whole run −56.3%, logmesquite complex −66.2% (the 37.4 Ir/elem K>=2 append_col wrap measured and NOT win-eating). DESIGN DETERMINATION (probe-verified): an Eigen column vector binds T_x DIRECTLY (is_matrix=is_eigen; T_x_rows=-1 general branch = exact K=1 design semantics) and a scalar binds T_beta — K=1 is a plain call, zero wrap. THE PASS: Optimize.emit_normal_glm, LAST in suite, ON AT EVERY LEVEL INCL. DEFAULT, reverse-mode only; matches matrix x*beta(+alpha) AND the scalar-intercept + slope·data-vector chain (incl. the O1-fma spelling, subtracted slopes, K>=2 via append_col + vector literal). PATTERN DISCIPLINE AT SCALE: 10 external negatives 0 fires at both levels; FULL-TREE census 1233 models — exactly 2 differing per level, all intended fires (default: the integration model + normal_id_glm_old_performance.stan; O1: + an inlined-function fma chain; roles verified); blr default = exactly its intended one-statement fire, O1 byte-identical (status quo preserved). STATISTICAL CLASS reported honestly: lp rel-L2 1.016e-16 (max 2.0 ulp) kidscore / 1.05e-16 logmesquite; grads 1.7e-15/6.1e-16, bitwise-differ (99-100/100); sampler 3x4 both arms: rep ratios 1.000/0.315/0.941 vs stock's own 96.5% rep spread, the rep-1 gap REFUTED by a long-horizon control (warmup 1000/samples 500: emit ESS 244 vs 137, rhat 1.012 vs 1.057); grad calls −0.13%; 0/12 draws md5 (stated). Branch glm-default-emit @ bc71346 (2 commits). Record: results/glm_default_emit_w128.md. FILING: stanc3#10 ([upstream-candidate]; upstream form = same pass gated --Oexperimental-first — changes default-level numerics). CONSEQUENCE: the W-119 premise inversion is now FIXED end-to-end for the normal family — everyday kidscore/logmesquite-class regressions reach the glm path at default level with a measured −56..−66% complex.

## 2026-08-29 — W-127 PRE-REGISTRATION (before any code; family 4): the ADDITIVE multi-gather bernoulli_logit eta shape — extend bernoulli_logit_lpmf_gathered's predictor space from the 2PL bilinear alpha[ii].*(theta[jj]-beta[ii]) to the election88-class sum of gathered coefficient vectors (+ optional data-vector terms): eta[n] = a1[g1[n]] + a2[g2[n]] + ... + xd[n]*ak[gk[n]]-style leaves. Gate model: election88_full (build/election88_full__default — 5 gathered coefficient vectors + 5 dense data terms, y ~ bernoulli_logit(y_hat), y_hat a TRANSFORMED PARAMETER (output columns! — the primitive/emission must keep y_hat materialized for output: value-only write in the tp loop, likelihood consumes the gathered form; the W-115 push-loop pattern applies). W-112 gate template verbatim (bit-identity class, model flags, throw-set parity, FMA-schedule match per W-108.1). Branch off gathered-glm-mapfix (the fixed tip) in external/math_dev_w127 (branch gathered-additive). INCREMENT 1 = primitive + hand-edit gate ONLY (emission registry row = increment 2, later). Deliverable: results/additive_gathered_w127.md. Machine: <=2 cores, one callgrind at a time (W-118 may re-engage; coordinate via ps), nice 19.

## 2026-08-30 — W-118 CLOSE-OUT (PI-arbitrated FROM ARTIFACTS — the agent completed all measurements at 23:05 then went silent post-measurement; PI takeover, no report was filed by the agent itself; its record file results/fused_interior_w118.md was found complete incl. gates + forensics): the fused single-pass normal interior — MODEL GATES PASS at the strongest level (base AND fused arms both reproduce the canonical archive md5s digit-for-digit: radon_pp 4a9ca349…, radon_var bbafc652…; the record's stopgate dirs corroborate); CALLGRIND: radon_pp T 12,713.8M → 8,905.3M = −29.94% (IN the −15..−30% band, top edge); radon_var T 1,367.0M → 1,187.9M = −13.10% (MARGINAL MISS below the −15 floor — disclosed: N=919 amortizes the fused-loop machinery less; the band was calibrated on radon_pp's N=12,573 per the W-117 audit). Unit gates + controls green per logs/ (gate_a builds both flag levels, ctrl_run_* all PASSED incl. ThrowSetParity); branch REBASED onto the W-112.2 TU commit (union resolution — both sides appended tests) and math#15 ADVANCED to 57fbe44f2a (now carries primitive + throw-set fix + TU + fused interior). C1 CONSEQUENCE: the fused interior lands at radon_pp −29.9% MORE on top of W-112 — per-element ~55-65 class achieved — the vectorized-form emission's last claimed advantage (61 Ir/elem composed vs the primitive's 110) is now CLOSED: the bit-identical primitive matches or beats the statistical-class alternative. C1 lane CLOSED without a run, as pre-staged.

### W-118 CLOSE-OUT CORRECTION (PI erratum, posted where claimed): the agent's record results/fused_interior_w118.md was COMPLETE (it finished all gates + the record at 23:05 before going silent — only its final chat report is missing). The authoritative numbers are G-basis (the pre-registered metric): radon_pp G 10,760.4M → 6,952.4M = −35.39% (EXCEEDS the band's favorable edge, disclosed in the record as favorable — the overdelivery is the redundant copy + a second traversal dying); radon_var G 647.2M → 468.4M = −27.63% IN BAND. The PI's initial −29.94/−13.10 figures were TOTAL-run Ir (T-basis, also recorded) and the "radon_var marginal miss" statement was WRONG — both models PASS on the pre-registered basis. Full gate set per the record: ALL FOUR GREEN (27,821 unit checks both flag levels incl. SIMD widths/remainders + ThrowSetParity; 12/12 + 12/12 draws md5 vs the frozen archive in BOTH arm pairs; 100-pt parity exact-zero; TU + controls). C1 closure consequence stands unchanged (the fused interior closes the vectorized-form emission's last advantage — bit-identical primitive at statistical-class cost).

## 2026-08-30 — W-127 CLOSE-OUT (PI-arbitrated): additive multi-gather bernoulli_logit (family 4, increment 1) — BIT-IDENTITY FULLY ACHIEVED, gates (a)/(b)/(d) ALL GREEN: 13,076 unit checks + 8 throw-set cases 0 mismatches at BOTH flag levels; election88 draws md5 d2e2f896… DIGIT-FOR-DIGIT incl. all 11,566 y_hat columns (the tp-output constraint honored); 100-pt parity exact-zero; valgrind-reproduced; TU 7/7 + controls green. GATE (c) MISSED UNFAVORABLY, OWNED: +11.49% (1,578 Ir/elem stock; family baseline ESTABLISHED as this increment's deliverable) — the pre-staged keep-tp-stock design double-computes (eta recompute 8.14e9 Ir > the 2.38e9 stock likelihood complex removed); THE MEASURED INCREMENT-2 PRIZE: the tp-loop complex = 30.22e9 Ir = 55.2% of the stock run. PR HELD (a slower-increment-1 story would mislead; file after increment 2 harvests the prize). NEW STANDING LESSON #3 (after FMA-flags and throw-set): SWEEP-ORDER RELATIVE TO OTHER EDGES is a bit-identity dimension — in priors-before-likelihood models, stock applies prior edges BETWEEN the likelihood edge and the tp chains; direct scatter reorders each coefficient's accumulation by one term (1 ulp, lp exact) — cured by writing increments into y_hat's varis with stock's exact edge arithmetic and letting the retained tp chains propagate. Also: stock's var*1.0 ALIAS (no vari; makes binary-data slope increments single fused FMAs — matched with an aliased branch + volatile-e anti-CSE barrier); a mixed-ABI hazard in the family bundle's prebuilt W-103-era bridgestan.o (heap corruption on wide-tp outputs — caught, rebuilt in-copy, disclosed). Branch gathered-additive @ 5267fb4858 (4 commits). Record: results/additive_gathered_w127.md.

## 2026-08-30 — W-129 PRE-REGISTRATION (before any code; family 4 increment 2 — the measured 55.2% prize): the stanc3 ADDITIVE emission incl. the TP-LOOP RESOLUTION — when the gathered-additive pattern fires on a tp-built predictor whose ONLY other use is the likelihood (y_hat: check ALL reads), rewrite (a) the tp loop to VALUE-ONLY writes (y_hat's output values preserved bitwise — same per-element expression order in double space; no var chain), (b) the likelihood to the scatter overload (the separately-certified likelihood-last shape), eliminating the 30.22e9-Ir tp chain. GATES: (a) pattern discipline (negative controls: y_hat read anywhere else — GQ, print, later statements; partial reads; nonlinear leaves); (b) regenerated election88 hpp vs a GATED HAND-EDIT of the same transformation (the increment-2 hand-edit is the reference: build it FIRST, gate it bit-identity — y_hat output values bitwise, all parameter adjoints bitwise, draws md5); (c) end-to-end: regenerated → build → draws md5 == the increment-1 stock reference d2e2f896… (the tp value-only rewrite must be output-invisible); (d) no-op elsewhere + dune green. COST (report, no band): expect the tp complex → ~0 and the net ≈ −40..−50% of the stock run (8.14e9 recompute also dies — the likelihood consumes gathered values); attribution vs the W-127 baseline. Branch off gathered-registry @ 50e8c9d (the registry carries the emission machinery) in external/stanc3_w129. Machine: <=2 cores, one callgrind at a time, nice 19. Deliverable: results/additive_emit_w129.md.

## 2026-08-30 — W-129 CLOSE-OUT (PI-arbitrated; STOPPED at the pre-registered build-first gate — the stop-clause DID ITS JOB): the mission's central claim REFUTED empirically and structurally — "the tp chains no longer exist, so the sweep-order problem disappears" is FALSE: the W-127 divergence is the DELIVERY POSITION of the likelihood's increments (stock delivers Sigma-w through TP-BLOCK varis = earliest frames, swept LAST, after every prior edge; a likelihood-site callback is swept BEFORE the priors preceding it). CAUSAL TRIANGLE (clean experiment): pure-stock + likelihood statement MOVED BEFORE the priors = gradients bitwise-IDENTICAL to stock (0/100) — statement order irrelevant; the fold divergence isolates ENTIRELY to callback creation position. CORRECTED APPLICABILITY: for tp-built predictors the scatter matches only with priors AFTER the likelihood (the header doc is right only for composed predictors like hier_2pl — correction to be posted on the header docs). GATE (b) FAIL as registered: lp/output 0/100 exact (the value-only rewrite IS output-invisible — that constraint HOLDS), adjoints 100/100 differ (~32/90 comps, rel-L2 ~1e-16), draws = the increment-1 scatter arm's md5. GATE (e) — THE PRIZE IS REAL AND BIGGER: 54.76e9 -> 23.73e9 Ir = −56.7% (wall 5.73->1.70s; tp complex -> ~0 exactly — vari pushes 4.898e9->1.3e6; log1p identical to the digit); "eta recompute -> 0" infeasible (the scatter overload's certified value path IS the recompute). No OCaml pass built (correct); stanc3_w129 clean at base. LESSON #4 (standing): callback CREATION POSITION is a bit-identity dimension — varis created in the tp block are swept last; the likelihood-site callback sweeps before preceding priors. TWO QUANTIFIED FOLLOW-UPS: (1) bit-identity lane (PI-CHOSEN, W-130 below): math-side per-element custom vari created AT THE TP LOOP (forward = the W-129-validated value-only path; chain replays the W-127-certified element backward; likelihood line stays STOCK — delivery position = stock's by construction), same ~−30e9 elimination; (2) statistical-class reclassify harvesting −56.7% now (the W-129 hand-edit already IS that arm — only the gate class changes; fallback if W-130 walls). Record: results/additive_emit_w129.md; artifacts scratch/w129/.

## 2026-08-30 — W-130 PRE-REGISTRATION (before any code; family-4 increment 2', the bit-identity lane): the TP-BLOCK custom-vari primitive — extend the gathered-additive header with a per-element vari created at the tp loop: forward = the W-129-validated value-only eta computation (in double space, y_hat output values bitwise); chain() = the W-127-certified element backward (increments into the coefficient varis with stock's exact edge arithmetic; delivery position = the tp block = swept last = stock's). The LIKELIHOOD LINE STAYS STOCK (bernoulli_logit_lpmf over the var y_hat — its edge application to the custom varis is stock-position arithmetic). Branch off gathered-additive @ 5267fb4858 in external/math_dev_w130 (branch gathered-additive-tpvari). GATES (W-112 template): (a) bitwise unit at MODEL FLAGS both levels — the custom-vari path vs fully-stock composed (tp loop + likelihood), lp + every adjoint + y_hat VALUES, plus throw-set; (b) MODEL: hand-edit election88's tp loop to the custom-vari construction (likelihood line untouched) -> draws md5 == d2e2f896… (STOCK reference), parity exact-zero; (c) callgrind: expect the tp complex -> ~0 with the same −55..−60% run class as W-129 measured (the elimination is the same; the vari machinery adds ~1 vari/elem vs the chain complex removed); (d) TU + controls. THEN W-131 (emission) on green. Machine: <=2 cores, one callgrind at a time, nice 19, resilience rules in force. Deliverable: results/tpvari_w130.md.

## 2026-08-30 — W-130 CLOSE-OUT (PI-arbitrated): the TP-BLOCK custom vari — ALL FOUR GATES GREEN and the session's biggest single-model number: election88 run 54,761.4M -> 17,788.2M Ir = −67.51% (band −55..−60 exceeded favorably; overshoot mechanism owned: the sweep/zeroing frames collapse with 1/23 the varis — bs_log_density_gradient self 4.36e9->0.35e9), wall 5.73->1.39s (4.1x), Ir/elem 1,578->513 — AND the bit-identical arm BEATS the W-129 statistical-class rewrite (23.73e9): bit-identity is the NATIVE class of the whole family-4 transformation. THE CONSTRUCTION: gathered_additive_tp — ONE additive_tp_vari per element pushed on the var_stack AT the tp loop (swept last = stock's delivery position BY CONSTRUCTION — lesson #4 answered structurally); forward = the W-129-validated value-only path; chain() replays the W-127-certified element backward (leaves reverse-declaration, intercept last); THE LIKELIHOOD LINE STAYS STOCK (its edge application digit-identical in the profile; edge-loop disassembly byte-identical; FMA provenance counts recorded). Design refinement owned: slot tags (slot_term/slot_slope_term/slot_slope2_term) in the leaf vocabulary — an rvalue view var cannot reach SoA slot destinations; per-layout discipline gate-certified. GATES: (a) 440,067 bitwise + 9 throw checks 0 mismatches at BOTH flag levels, beta layout-varied, P6 priors-before AND P7 priors-after BOTH exact (the W-129 failure case); (b) draws md5 d2e2f896… DIGIT-FOR-DIGIT incl. 11,566 y_hat cols + parity exact-zero; (d) TU 11/11 + controls. Branch gathered-additive-tpvari @ a2593a12fe (contains the family-4 line). Record: results/tpvari_w130.md. FILING: math#22 (the full additive family: primitive + scatter overload + tp-vari factory). W-131 (emission) = "tp loop -> the factory call", no likelihood rewrite — PRE-REGISTERED + LAUNCHING.

## 2026-08-30 — W-131 PRE-REGISTRATION (before any code; family 4 increment 3): the ADDITIVE emission — a registry entry matching the tp-built gathered-additive predictor pattern (loop-local elementwise y_hat from gathered leaves + data terms, ONLY other use the likelihood) and emitting gathered_additive_tp factory calls in the tp loop; NO likelihood rewrite. GATES: (a) negatives (y_hat read elsewhere: GQ/print/later/partial; nonlinear leaves; scalar; O0); (b) regenerated election88 hpp ≡ the W-130-GATED hand-edit (the reference exists and is GREEN — the strongest possible emission gate); (c) end-to-end: regenerated -> build (bundle carrying the tpvari header) -> draws md5 == d2e2f896… + parity exact-zero; (d) no-op elsewhere + dune green. Branch off gathered-registry @ 50e8c9d in external/stanc3_w131. Machine: <=2 cores, one callgrind at a time, nice 19. Deliverable: results/additive_emit_w131.md.

## 2026-08-30 — W-131 CLOSE-OUT (PI-arbitrated): the additive emission — ALL FOUR GATES GREEN; FAMILY 4 COMPLETE END-TO-END (math primitive + scatter overload + tp-vari factory [math#22] + automatic emission). The emission lands in the strongest class yet: the compiler output alone reproduces the W-130-gated construction BIT-FOR-BIT (factory statement token-identical 127/127; end-to-end draws md5 d2e2f896… digit-for-digit incl. all 11,566 y_hat cols; parity 0/100 exact-zero; FMA provenance 305/42/23 = the gated arm exactly; wall 1.67s vs stock 5.73s). Pattern discipline at scale: 13 negatives never fire; full-tree census 2,562 models at --O1 = exactly 3 fires in 2 models, all intended (incl. the in-repo election88 verbatim copy, the W-115 precedent); no-op elsewhere incl. W-128's models. KEY FINDING: the default-vs-O1 double-path conflict dissolves — GCC fp-contraction makes the source-level fma difference vanish at model flags; the stock-O1 arm (00bf1d08…, var-fma trajectory) stands as the level-attribution control. Owned deviation: the hand-edit's runtime-inert current_statement__=13 line not printed (one statement = one label; labels stay allocated). Branch gathered-additive-tp-emit @ 03b5180 (4th registry row + the TpLoop emission class + the matcher for plain and O1-fma-fused eta spellings). Record: results/additive_emit_w131.md. FILING: stanc3#11 (extends #8; REQUIRES math#22).

## 2026-08-30 — W-126 PRE-REGISTRATION (before any code; family 3 — the LAST gathered family): the pcm/ordered gathered primitive — gate model gpcm_latent_reg_irt (build/gpcm_latent_reg_irt__default; the W-80 polytomous-IRT harm model doubles as the gate). Loop form: for (n) target += pcm(y[n], theta[jj[n]] .* alpha[ii[n]], kappa-args) — per-observation ETA IS A CATEGORY VECTOR (the gathered bilinear is a diagonal fill), interior = the partial-credit LSE over categories (the campaign's hardest interior). W-112 gate template verbatim (bit-identity class: model flags both levels; throw-set parity; contraction matching; the per-observation-var/accumulator pattern as needed). Branch off fork/develop in external/math_dev_w126 (branch gathered-pcm). INCREMENT 1 = primitive + hand-edit gate ONLY (emission row later). Read stock pcm_lpmf's interior FIRST and state whether the LSE reduction is replicable bit-identically (Eigen redux semantics decide; if provably not, STOP and report — statistical-class escalation is the PI's call). Machine: <=2 cores, one callgrind at a time, nice 19, resilience rules in force. Deliverable: results/pcm_gathered_w126.md.

## 2026-08-30 — W-126 CLOSE-OUT (PI-arbitrated): family 3, the pcm/ordered gathered primitive — ALL FOUR GATES GREEN with the CAMPAIGN'S BIGGEST NUMBER: gpcm run 214.3e9 -> 25.1e9 Ir = −88.28% (Ir/grad 46.05M -> 5.40M; wall 22.6->2.3s = 9.7x), band −25..−45 exceeded favorably — mechanism: stock's dominant cost is a 115.8e9 mis-symbolized copy complex (per-observation expression materialization inside the stock softmax path) collapsing with the graph (W-130-class compounding). STEP ZERO: REPLICABLE — with two findings: (1) NO stock pcm_lpmf exists — the gate model's likelihood is a USER FUNCTION (softmax(cumulative_sum(append_row(0, theta*alpha − beta))) + categorical_lpmf); the 'stock interior' is that composed generated path; (2) the softmax interior is EXPRESSION-TYPE- AND STACK-DEPENDENT (lazy val-view = scalar traversal/glibc exp; dense = packetized pexp; Eigen 5 flips) — resolved by calling softmax through the EXACT view instantiation stock hits (probe-verified 0 diffs on both stacks). GATES: (a) 802 cases / 20,764 component checks + 13 throws, 0 mismatches at BOTH flag levels (real gpcm N=5500, K=2..8, 18 layouts); (b) stock ref a342848b… DOUBLY anchored (the W-80 shipped .so on cmdstan-2.39 reproduces it); prim arm digit-for-digit; parity 0/100 exact-zero (D=530, DC=545); (d) TU 5/5 ON EIGEN 5 (a genuine cross-stack certification) + controls green. Deviations owned: the near-frozen reference trajectory (W-80-documented 510-exception spam; byte-identical both arms; parity carries the gradient proof); the TU caught the W-129 delivery-position mechanism LIVE in family 3 (test-side ordering bug, fixed). Branch gathered-pcm @ e355b14535. Record: results/pcm_gathered_w126.md. FILING: math#23. INCREMENT 2 (the composed-user-function matcher — a NOVEL matcher class) = GO, pre-registered as W-132.

## 2026-08-30 — W-132 PRE-REGISTRATION + W-133 PRE-REGISTRATION (the final two lanes): W-132 = family-3 EMISSION — a stanc3 matcher for the COMPOSED USER-FUNCTION pattern (the softmax(cumulative_sum(append_row(0, gathered-bilinear))) + categorical_lpmf chain — matching through a user function body is a novel matcher class; the registry row emits the W-126 primitive; gates: negatives incl. user functions with extra uses of the intermediate vectors, non-gathered bilinears, O0; regenerated gpcm hpp ≡ the W-126 hand-edit; end-to-end md5 a342848b…; no-op elsewhere + dune). Branch off gathered-registry @ 50e8c9d in external/stanc3_w132; requires math#23. W-133 = THE WRAP BENCHMARK (analysis-first): assemble the everything-v2 table from EXISTING measurements — E-prime per gather-family model (hier_2pl W-108.1, radon_pp/var W-112.2+W-118, bym2 W-113) + the W-109 E/S archive for the rest + the supplementary statistical-class arms flagged separately (kidscore/logmesquite W-128; election88/gpcm are non-CORE, presented as a side table); recompute geomeans with conventions stated; NO new sampling unless a specific number is missing and cheap; deliverable results/everything_v2_w133.md — the promotion-decision artifact.

## 2026-08-30 — W-133 CLOSE-OUT (PI-arbitrated): the everything-v2 WRAP TABLE — the promotion-decision artifact, assembled analysis-first (zero new sampling): everything-v2 E'/S = 1.746x geomean (band 1.728-1.761; hier_2pl dispatch-matched 5.54x used conservatively among three stated readings) vs W-109's 1.485x; DECOMPOSITION: the 4 primitive models = 3.19x geomean MEASURED (reproduces the W-112.2 close-out exactly), the 17 others = 1.515x unchanged-by-construction (bit-identity => E == E'); every E' cell md5-identical to its W-109 archive cell; absolutes 38.81 -> 57.61 -> 67.75 ESS/s; wall-convention audit clean (all sources use the per-rep stanza-sum convention; no conversion needed). NOT in E' (conservative): the W-118 fused interior (radon cells pre-fusion). Supplementary arms properly classed: glm emission STATISTICAL (kidscore -56.3%, logmesquite -66.2%, flagged, cannot merge); election88 4.1x / gpcm 9.7x BIT-IDENTICAL (non-CORE side table). THE PROMOTION PARAGRAPH (the decision artifact, verbatim in the record): full stack = 1.746x measured, every draw bit-identical wherever a transform fires; components (1) W-109 posture 1.485x configuration-class (+E+ 1.44x optional), (2) the gathered primitives 3.19x on their models + the side wins, md5-proven, (3) automatic emission, (4) glm emission statistical-class -56..-66%, (5) Package A = robustness not ESS/s (0.03-0.22x R0) with the W-125-recommended budget floor max(64, 16F/5) cap 128. Record: results/everything_v2_w133.md. REMAINING: W-132 (family-3 emission) + the session refresh.

## 2026-08-30 — W-132 CLOSE-OUT (PI-arbitrated) — THE CAMPAIGN'S FINAL LANE: family-3 emission — ALL FOUR GATES GREEN: the composed USER-FUNCTION matcher proved EASY post-inlining (function_inlining is the suite's FIRST pass — gathered_families sees the pcm body fully expanded; gensym-fresh inline locals make the no-extra-uses side condition STRUCTURAL); the loop rewrites to pcm_lpdf_gathered<propto__> + per-term pushes. GATES: 8 negatives 0 fires; census 1,798 two-arm compiles = 1,797 byte-identical + exactly 1 firing model (2 intended fires) + whole-tree closure by source grep; emitted-vs-parent = exactly 2 hunks, 34/34 tokens identical to the W-126 hand-edit (cosmetic label classes per W-131/W-115); END-TO-END: compiler output alone reproduces draws md5 a342848b… DIGIT-FOR-DIGIT (510-error pattern identical) + 100-pt parity EXACT-ZERO; no-op elsewhere; dune green. KEY FINDINGS: the reference md5 is LEVEL-INSENSITIVE (parent at default AND --O1 both reproduce it — gate (c) held literally with no attribution table); a real operand-dropping bug (the W-115 ICAR class — theta emitted in both slots on commuted spellings) caught by the control matrix and fixed BEFORE any recorded gate; neg_seg_other_index reclassified intended (faithful swapped binding, disclosed). Branch gathered-pcm-emit @ d879d68. Record: results/pcm_emit_w132.md. FILING: stanc3#12 (requires math#23).

## 2026-08-30 — THE GATHERED-GLM CAMPAIGN IS COMPLETE (W-108..W-133, one session-arc): all four families END-TO-END (primitive + automatic emission), ALL BIT-IDENTICAL — hier_2pl-class (−41%→−67% composed), radon-class (−65%→−77% fused; throw-set fixed), ICAR (−17%), additive/tp-vari (−67.5%, 4.1×), pcm (−88.3%, 9.7×); the everything-v2 wrap = 1.746× geomean ESS/s with every draw md5-identical wherever a transform fires; 4 standing bit-identity lessons banked (model-flag gates/FMA contraction; throw-set parity; sweep-order; creation position); 11 fork PRs + 1 issue filed; the statistical-class levers (glm emission −56..−66%, const-hoist −48%) separately quantified with the re-add remedy validated
