...
inline auto multiply(T1&& A, T2&& B) {
  ...
  if constexpr (is_autodiff_v<T2> && is_autodiff_v<T1>) {
    ...
    if constexpr (is_eigen_v<return_t>) {
      ...
      // access on Product expressions and is probe-verified bitwise-equal to
      // stock's implicitly-evaluated temp.
      arena_t<plain_type_t<decltype(arena_A * arena_B.val_op())>> res_val
          = arena_A * arena_B.val_op();
      arena_t<return_t> res(res_val.rows(), res_val.cols());
      make_nochain_vari_array(res_val, res.data());
    ...
    if constexpr (is_eigen_v<return_t>) {
      ...
      // access on Product expressions and is probe-verified bitwise-equal to
      // stock's implicitly-evaluated temp.
      arena_t<plain_type_t<decltype(arena_A.val_op() * arena_B)>> res_val
          = arena_A.val_op() * arena_B;
      arena_t<return_t> res(res_val.rows(), res_val.cols());
      make_nochain_vari_array(res_val, res.data());
