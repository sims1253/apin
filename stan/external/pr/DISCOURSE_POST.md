...
- **[math #2 — square() should multiply, not call std::pow(x, 2)](https://github.com/sims1253/math/pull/2).** Under default `-fmath-errno` the compiler cannot remove the call, so it stays a ~105-instruction libm call in hot paths: −9–15% per gradient on GP models. Bonus finding: glibc's `pow(x,2)` is 1 ulp off the correctly rounded product about 0.08% of the time, so the multiply is the *more accurate* operation.
